...
def gifify(png, path_prefix='png/', n_w=2, n_h=2):
    ...
    # make small version
    os.system('convert -delay 5 -dispose previous -loop 0 tmp/*_small.gif gif/%s_small.gif' % png.replace('.png', ''))
    os.system('rm tmp/*small.gif')
    # make big version
    os.system('convert -delay 5 -dispose previous -loop 0 tmp/*.gif gif/%s.gif' % png.replace('.png', ''))

    os.system('rm tmp/*png && rm tmp/*gif')

hard_pngs = [f for f in os.listdir('bunny') if f.endswith('hahaha.png')]
print hard_pngs
for hard_png in hard_pngs:
    gifify(hard_png, path_prefix='bunny/', n_w=3, n_h=3)
